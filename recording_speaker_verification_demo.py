# -*- coding: utf-8 -*-
"""
Created on Tue Jan  4 11:50:37 2022
...
sf.write('new.wav', data, samplerate, subtype='PCM_S8')

print("Done")
ob = sf.SoundFile('new.wav')
...
model_file= 'test_deploy/model_raw.pkl' # This is the model to use for computing the d-vectors (it should be pre-trained using the speaker-id DNN)
cfg_file=r'cfg/SincNet_TIMIT.cfg' # Config file of the speaker-id experiment used to generate the model
te_lst=r'test_deploy/new.scp' # List of the wav files to process
...
speaker1=list(d_vect_dict.values())


speaker2=[ 0.02393546,  0.01114575,  0.00630309, -0.00280812,
       -0.00380172,  0.01185615]

first_speaker=np.reshape(speaker1, (-1, 1)).T
second_speaker=np.reshape(speaker2, (-1, 1)).T

result_cos_sim=cosine_similarity(speaker1,speaker2)
print(result_cos_sim)
if result_cos_sim>=0.5:
    print(first_speaker,"and",second_speaker, "are the same")
elif result_cos_sim<0.5:
    print(first_speaker,"and",second_speaker, "are not the same")     
